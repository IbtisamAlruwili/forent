...
export default function Courses({ token }) {
  ...
  useEffect(async () => {
    const res = await axios.get("https://ibtisam-b.herokuapp.com/getaddProduct", {
  ...
  const addProduct = async () => {
    ...
    try {
      const result = await axios.post(
        "https://ibtisam-b.herokuapp.com/addProduct",
        {
          name: name,
          price: price,
          img: img,
  ...
  const deletePro = async (id, index) => {
    try {
      const deletedCourse = await axios.delete(
        `https://ibtisam-b.herokuapp.com/deletProduct/${id}`,
        {
          headers: { authorization: "Bearer " + token },
        }
      );
  ...
  const fav = async (id) => {
    try {
      const result = await axios.post(
        `https://ibtisam-b.herokuapp.com/like/${id}`,
        {},
        {
          headers: { authorization: "Bearer " + token },
        }
